...
pub fn range_compare<T: PrimInt>(
    left: &core::ops::RangeInclusive<T>,
    right: &core::ops::RangeInclusive<T>,
) -> RangeCompare {
...
pub fn intersection<T: PrimInt>(
    ...
    if RangeDisjoint::compare(left, right).is_none() {
        core::cmp::max(*left.start(), *right.start())..=core::cmp::min(*left.end(), *right.end())
    } else {
        T::one()..=T::zero()
    }
}
                ...
                if left.end() < right.start() || right.end() < left.start() {
                    ...
                    Some(if left == right {
                        ...
                        match left.start().cmp(right.start()) {
                            core::cmp::Ordering::Less => match left.end().cmp(right.end()) {
                                core::cmp::Ordering::Less => RangeIntersect::OverlapsLeft,
                                core::cmp::Ordering::Equal => RangeIntersect::ContainsLast,
                                core::cmp::Ordering::Greater => RangeIntersect::ContainsProper,
                            },
                            core::cmp::Ordering::Equal => match left.end().cmp(right.end()) {
                                core::cmp::Ordering::Less => RangeIntersect::ContainedByFirst,
                                core::cmp::Ordering::Equal => unreachable!(),
                                core::cmp::Ordering::Greater => RangeIntersect::ContainsFirst,
                            },
                            core::cmp::Ordering::Greater => match left.end().cmp(right.end()) {
                                core::cmp::Ordering::Less => RangeIntersect::ContainedByProper,
                                core::cmp::Ordering::Equal => RangeIntersect::ContainedByLast,
                                core::cmp::Ordering::Greater => RangeIntersect::OverlapsRight,
                            },
                        }
                    })
                }
