...
pub use range_compare::{RangeCompare, RangeDisjoint, RangeIntersect, intersection, range_compare};
...
where
    A: smallvec::Array + Eq + core::fmt::Debug,
    A::Item: Clone + Eq + core::fmt::Debug,
{
    ranges: SmallVec<A>,
}
...
where
    A: smallvec::Array<Item = RangeInclusive<T>> + Eq + core::fmt::Debug,
    T: 'a + PrimInt + core::fmt::Debug,
{
    ...
    T: PrimInt + core::fmt::Debug,
{
    ...
    pub fn insert_range(&mut self, range: A::Item) -> Option<Self> {
        ...
        match (before, after) {
            ...
            (None, None) => {
                ...
                let new_range = core::cmp::min(*range.start(), *self.ranges[0].start())
                    ..=core::cmp::max(*range.end(), *self.ranges[self.ranges.len() - 1].end());
                self.ranges.clear();
                self.ranges.push(new_range);
                if !isect.is_empty() { Some(isect) } else { None }
            }
            ...
            (Some(before), None) => {
                if before + 1 == self.ranges.len() {
                    ...
                    self.ranges[before + 1] =
                        core::cmp::min(*range.start(), *self.ranges[before + 1].start())
                            ..=core::cmp::max(
                                *range.end(),
            ...
            (None, Some(after)) => {
                if after == 0 {
                    ...
                    self.ranges[0] = core::cmp::min(*range.start(), *self.ranges[0].start())
                        ..=core::cmp::max(*range.end(), *self.ranges[after - 1].end());
        ...
        A: Clone,
    {
        ...
        other.ranges.iter().cloned().for_each(|r| {
            new.insert_range(r);
        });
        new
    }
    ...
    fn binary_search_before(&self, range: &A::Item) -> Option<usize> {
        ...
        if found { Some(before) } else { None }
    }
    ...
    fn range_intersection(&self, range: &A::Item, range_range: core::ops::Range<usize>) -> Self {
...
where
    A: smallvec::Array + Eq + core::fmt::Debug,
    A::Item: Clone + Eq + core::fmt::Debug,
    B: smallvec::Array<Item = A::Item> + Eq + core::fmt::Debug,
{
...
where
    A: smallvec::Array<Item = RangeInclusive<T>> + Eq + core::fmt::Debug,
    T: PrimInt + core::fmt::Debug + serde::Serialize,
{
...
where
    A: smallvec::Array<Item = RangeInclusive<T>> + Eq + core::fmt::Debug,
    T: PrimInt + core::fmt::Debug + serde::Deserialize<'de>,
{
